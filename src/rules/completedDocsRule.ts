...
export class Rule extends Lint.Rules.TypedRule {
    ...
    private readonly requirementFactory = new ExclusionFactory();

    public applyWithProgram(sourceFile: ts.SourceFile, program: ts.Program): Lint.RuleFailure[] {
        const options = this.getOptions();
        const completedDocsWalker = new CompletedDocsWalker(sourceFile, options, program);

        completedDocsWalker.setRequirementsMap(this.getRequirementsMap(options.ruleArguments));

        return this.applyWithWalker(completedDocsWalker);
    }

    private getRequirementsMap(ruleArguments: Array<DocType | IExclusionDescriptors>): Map<DocType, Array<Exclusion<any>>> {
        if (ruleArguments.length === 0) {
            ruleArguments = [Rule.defaultArguments];
        }

        return this.requirementFactory.constructExclusionsMap(ruleArguments);
    }
}
...
class CompletedDocsWalker extends Lint.ProgramAwareRuleWalker {
    ...
    private requirementsMap: Map<DocType, Array<Exclusion<any>>>;

    public setRequirementsMap(requirementsMap: Map<DocType, Array<Exclusion<any>>>): void {
    ...
    private checkNode(node: ts.Declaration, nodeType: DocType): void {
        ...
        const requirements = this.requirementsMap.get(nodeType);
        if (!requirements) {
            return;
        }

        for (const requirement of requirements) {
            if (requirement.excludes(node)) {
                return;
            }
        }
